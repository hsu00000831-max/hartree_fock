...
def compute_densities(ek_list, U, ns, mu, beta, field):
    ...
    for hk in ek_list:
        H = hk.copy()
        # 加入論文 Hint 的 -0.5 修正
        H[0, 0] += U * (n1dn_in - 0.5) - field - mu
        H[1, 1] += U * (n1up_in - 0.5) + field - mu
        H[2, 2] += U * (n2dn_in - 0.5) - field - mu
        H[3, 3] += U * (n2up_in - 0.5) + field - mu
        rho = F_of_H(H, beta)
...
def func_mu(x, ek_list, U, n1up, n1dn, n2up, n2dn, beta, field, n_target):
    ...
    for hk in ek_list:
        H = hk.copy()
        # 加入論文 Hint 的 -0.5 修正
        H[0, 0] += U * (n1dn - 0.5) - field - mu
        H[1, 1] += U * (n1up - 0.5) + field - mu
        H[2, 2] += U * (n2dn - 0.5) - field - mu
        H[3, 3] += U * (n2up - 0.5) + field - mu
...
def run_scf(ek_list, U, n_target, inital_guess, beta, maxiter=500): 
    ...
    # 關鍵修正：因為加入了 -0.5，系統的半填充能量中心回到了 0
    # 所以初始化學勢猜測直接設為 0.0 即可
    mu0 = 0.0
...
if __name__ == "__main__":
    ...
    # 2. 設定你要測試的「單一參數點」
    # ==========================================
    U_target = 15       # <--- 在這裡修改你要測試的 U
    delta_target = -0.2  # <--- 在這裡修改你要測試的摻雜濃度 delta
    beta = 200.            # 溫度參數
